...
const SidebarItem = ({ name, icon: Icon, isActive, onClick, hasChildren, isExpanded }: { name: string, icon: any, isActive: boolean, onClick: () => void, hasChildren?: boolean, isExpanded?: boolean }) => (
  <button
    ...
    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 group ${
      isActive 
        ? 'bg-icy-main text-white shadow-lg shadow-icy-main/25' 
        : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5'
    }`}
  >
    <div className="flex items-center gap-3">
        <div className={`${isActive ? 'text-white' : 'text-gray-400 dark:text-gray-500 group-hover:text-icy-main'}`}>
          <Icon size={18} />
        </div>
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className="lg:w-64 flex-shrink-0">
      <div className="sticky top-24 space-y-1 bg-white dark:bg-[#002466]/20 p-4 rounded-2xl border border-gray-200 dark:border-white/5 backdrop-blur-xl">
                              ...
                              {catServices.map(sub => (
                                  <button
                                      ...
                                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-between ${
                                          ...
                                          ? 'bg-icy-main/10 text-icy-main' 
                                          : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5'
                                      }`}
                                  >
                                      {sub.title}
                                      {selectedService === sub.id && <div className="w-1.5 h-1.5 rounded-full bg-icy-main" />}
                                  </button>
                              ))}
                          </motion.div>
                      )}
        ...
        <div className="my-4 border-t border-gray-200 dark:border-white/10 mx-4" />
        
        <div className="p-4 rounded-xl bg-gradient-to-br from-icy-deep to-icy-main text-white">
          <h4 className="font-bold text-sm mb-1">Riona AI</h4>
          <p className="text-xs text-white/80 mb-3">Your campaigns need attention.</p>
