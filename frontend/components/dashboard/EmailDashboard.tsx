          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={deliverabilityData}>
                ...
                <YAxis yAxisId="left" domain={[90, 100]} axisLine={false} tickLine={false} tick={{fill: '#9ca3af'}} label={{ value: 'Delivered %', angle: -90, position: 'insideLeft', fill: '#9ca3af', style: {fontSize: '10px'} }} />
                <YAxis yAxisId="right" orientation="right" domain={[0, 5]} axisLine={false} tickLine={false} tick={{fill: '#9ca3af'}} label={{ value: 'Bounce/Spam %', angle: 90, position: 'insideRight', fill: '#9ca3af', style: {fontSize: '10px'} }} />
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <div className="flex bg-gray-100 dark:bg-white/5 p-1 rounded-xl">
                <button 
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'campaigns' ? 'bg-white dark:bg-icy-main text-black dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                >
                    One-Off Campaigns
                </button>
                <button 
                    onClick={() => setActiveTab('flows')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'flows' ? 'bg-white dark:bg-icy-main text-black dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                >
                    Automated Flows
                </button>
            </div>
            <button className="flex items-center gap-2 px-4 py-2 bg-icy-main text-white rounded-lg text-sm font-bold hover:bg-icy-secondary transition-colors">
                        ...
                        emailFlows.map((f, i) => (
                            <tr key={i} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-4 py-3">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${f.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                                        {f.status}
                                    </span>
                                </td>
